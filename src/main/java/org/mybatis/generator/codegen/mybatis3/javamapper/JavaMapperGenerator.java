...
import org.mybatis.generator.codegen.mybatis3.javamapper.elements.BatchUpdateByPrimaryKeyWithoutBLOBsMethodGenerator;
...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
	...
	public List<CompilationUnit> getCompilationUnits() {
		String progressStr = getString("Progress.17", introspectedTable.getFullyQualifiedTable().toString());
		...
		if (stringHasValue(rootInterface)) {
			...
			if (introspectedTable.getBaseRecordDtoType() != null) {
				FullyQualifiedJavaType modelDtoType = new FullyQualifiedJavaType(
						introspectedTable.getBaseRecordDtoType());
				fqjt.addTypeArgument(modelDtoType);
				interfaze.addImportedType(modelDtoType);
			}
		...
		addBatchUpdateByPrimaryKeyWithBLOBsMethod(interfaze);
		addUpdateByPrimaryKeyWithoutBLOBsMethod(interfaze);
		addBatchUpdateByPrimaryKeyWithoutBLOBsMethod(interfaze);
	...
	protected void addBatchUpdateByPrimaryKeyWithBLOBsMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateUpdateByPrimaryKeyWithBLOBs()) {
	...
	protected void addBatchUpdateByPrimaryKeyWithoutBLOBsMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateUpdateByPrimaryKeyWithoutBLOBs()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new BatchUpdateByPrimaryKeyWithoutBLOBsMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}
