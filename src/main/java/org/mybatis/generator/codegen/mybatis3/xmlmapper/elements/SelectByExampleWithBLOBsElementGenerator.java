...
public class SelectByExampleWithBLOBsElementGenerator extends AbstractXmlElementGenerator {
	...
	public void addElements(XmlElement parentElement) {
		...
		if (associationRootTable.getChildren().size() > 0) {
			...
			XmlElement blobColumnList = new XmlElement("include");
			blobColumnList.addAttribute(new Attribute("refid", associationRootTable.getBlobColumnListId()));
			answer.addElement(blobColumnList);
	...
	private void generateIncludeColumnSql(AssociationRootTable associationRootTable, XmlElement answer) {
		for (AssociationTable associationTable : associationRootTable.getChildren()) {
			...
			if (associationTable.getIntrospectedTable().getRules().generateBlobColumnList()) {
				ifElement.addElement(new TextElement(","));
				XmlElement blobColumnRef = new XmlElement("include");
				blobColumnRef.addAttribute(new Attribute("refid", associationTable.getBlobColumnListId()));
				ifElement.addElement(blobColumnRef);
			}
