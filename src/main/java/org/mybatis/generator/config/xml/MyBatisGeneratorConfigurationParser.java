...
public class MyBatisGeneratorConfigurationParser {
	...
	private void parseContext(Configuration configuration, Node node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			} else if ("javaModelDtoGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
			...
			} else if ("blankSqlMapGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseBlankSqlMapGenerator(context, childNode);
			} else if ("javaClientGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseJavaClientGenerator(context, childNode);
			} else if ("blankJavaClientGenerator".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseBlankJavaClientGenerator(context, childNode);
	...
	protected void parseSqlMapGenerator(Context context, Node node) {
		...
		String overwriteStr = attributes.getProperty("overwrite");
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			sqlMapGeneratorConfiguration.setOverwrite(overwrite);
		}
	...
	protected void parseBlankSqlMapGenerator(Context context, Node node) {
		BlankSqlMapGeneratorConfiguration sqlMapGeneratorConfiguration = new BlankSqlMapGeneratorConfiguration();

		context.setBlankSqlMapGeneratorConfiguration(sqlMapGeneratorConfiguration);
	...
	protected void parseTable(Context context, Node node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			} else if ("foreignKey".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseForeignKey(tc, childNode);
	...
	private void parseForeignKey(TableConfiguration tc, Node node) {
		Properties attributes = parseAttributes(node);

		String column = attributes.getProperty("column");
		String table = attributes.getProperty("table");
		String keyColumn = attributes.getProperty("keyColumn");
		if (column == null || table == null || keyColumn == null) {
			throw new RuntimeException("foreginKey's attribute column,keyColumn,table can't be null");
		}
		String mappedField = attributes.getProperty("mappedField");
		String useAssociation = attributes.getProperty("useAssociation");
		ForeginKey fk = new ForeginKey();
		fk.setColumn(column);
		fk.setTable(table);
		fk.setKeyColumn(keyColumn);
		fk.setMappedField(mappedField);
		if (!"false".equalsIgnoreCase(useAssociation)) {
			useAssociation = "true";
		}
		fk.setUseAssociation(Boolean.parseBoolean(useAssociation));
		parseForeginKeyReference(fk, node);
		tc.addForeginKey(fk);
	}

	private void parseForeginKeyReference(ForeginKey parent, Node node) {
		NodeList nodeList = node.getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
			Node childNode = nodeList.item(i);
			if (childNode.getNodeType() != Node.ELEMENT_NODE) {
			...
			if ("foreignKey".equals(childNode.getNodeName())) {
				if (parent.isUseAssociation()) {
					throw new RuntimeException("if a foregin key has some foregin keys,"
							+ "this foregin key can't use the foregin table's association,"
							+ "useAssociation attribute should be false or not be declared");
				}
				...
				fk.setUseAssociation(Boolean.parseBoolean(useAssociation));
				fk.setParent(parent);
				parent.getChildren().add(fk);
				parseForeginKeyReference(fk, childNode);
			}
	...
	protected void parseJavaModelGenerator(Context context, Node node) {
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			javaModelGeneratorConfiguration.setOverwrite(overwrite);
		}
	...
	protected void parseJavaModelDtoGenerator(Context context, Node node) {
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			javaModelDtoGeneratorConfiguration.setOverwrite(overwrite);
		}
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseProperty(javaModelDtoGeneratorConfiguration, childNode);
			}
		}
	}
	...
	private void parseJavaClientGenerator(Context context, Node node) {
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			javaClientGeneratorConfiguration.setOverwrite(overwrite);
		}
	...
	private void parseBlankJavaClientGenerator(Context context, Node node) {
		BlankJavaClientGeneratorConfiguration blankJavaClientGeneratorConfiguration = new BlankJavaClientGeneratorConfiguration();

		context.setBlankJavaClientGeneratorConfiguration(blankJavaClientGeneratorConfiguration);
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			blankJavaClientGeneratorConfiguration.setOverwrite(overwrite);
		}
		blankJavaClientGeneratorConfiguration.setConfigurationType(type);
		blankJavaClientGeneratorConfiguration.setTargetPackage(targetPackage);
		blankJavaClientGeneratorConfiguration.setTargetProject(targetProject);
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if ("property".equals(childNode.getNodeName())) { //$NON-NLS-1$
				parseProperty(blankJavaClientGeneratorConfiguration, childNode);
			}
		}
	}
	...
	private void parseJavaControllerGenerator(Context context, Node node) {
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			javaControllerGeneratorConfiguration.setOverwrite(overwrite);
		}
	...
	private void parseJavaServiceGenerator(Context context, Node node) {
		...
		if (overwriteStr != null) {
			boolean overwrite = Boolean.parseBoolean(overwriteStr);
			javaServiceGeneratorConfiguration.setOverwrite(overwrite);
		}
