...
import org.mybatis.generator.config.foregin.AssociationUtils;
import org.mybatis.generator.config.foregin.ForeginKey;
...
public class Context extends PropertyHolder {
	...
	private BlankSqlMapGeneratorConfiguration blankSqlMapGeneratorConfiguration;
	...
	private BlankJavaClientGeneratorConfiguration blankJavaClientGeneratorConfiguration;
	...
	public BlankJavaClientGeneratorConfiguration getBlankJavaClientGeneratorConfiguration() {
		return blankJavaClientGeneratorConfiguration;
	}
	...
	public BlankSqlMapGeneratorConfiguration getBlankSqlMapGeneratorConfiguration() {
		return blankSqlMapGeneratorConfiguration;
	}
	...
	public void validate(List<String> errors) {
		...
		if (blankJavaClientGeneratorConfiguration != null) {
			blankJavaClientGeneratorConfiguration.validate(errors, id);
		}
	...
	public void setBlankJavaClientGeneratorConfiguration(
			BlankJavaClientGeneratorConfiguration blankJavaClientGeneratorConfiguration) {
		this.blankJavaClientGeneratorConfiguration = blankJavaClientGeneratorConfiguration;
	}
	...
	public void setJavaModelDtoGeneratorConfiguration(
			JavaModelDtoGeneratorConfiguration javaModelDtoGeneratorConfiguration) {
	...
	public void setBlankSqlMapGeneratorConfiguration(BlankSqlMapGeneratorConfiguration sqlMapGeneratorConfiguration) {
		this.blankSqlMapGeneratorConfiguration = sqlMapGeneratorConfiguration;
	}
	...
	public Map<String, IntrospectedTable> getTables() {
		Map<String, IntrospectedTable> tables = new HashMap<String, IntrospectedTable>();
		for (IntrospectedTable introspectedTable : introspectedTables) {
			String tableName = introspectedTable.getFullyQualifiedTable().getIntrospectedTableName();
			tables.put(tableName, introspectedTable);
		}
		return tables;
	}
	...
	public void initializeAssociation() {
		Map<String, IntrospectedTable> tables = getTables();
		for (IntrospectedTable introspectedTable : introspectedTables) {
			List<ForeginKey> fks = introspectedTable.getTableConfiguration().getForeginKeys();
			AssociationRootTable rootTable = new AssociationRootTable();
			String alias = introspectedTable.getTableConfiguration().getAlias();
			if (alias == null) {
				String domainObjectName = introspectedTable.getFullyQualifiedTable().getDomainObjectName();
				alias = lowerCaseFirstChar(domainObjectName);
			}
			rootTable.setAlias(alias);
			String tableName = introspectedTable.getFullyQualifiedTable().getIntrospectedTableName();
			rootTable.setName(tableName);
			rootTable.setIntrospectedTable(introspectedTable);
			rootTable.setBaseColumnListId(AssociationUtils.getLinkedColumnListId(rootTable,
					introspectedTable.getBaseColumnListId()));
			rootTable.setBlobColumnListId(AssociationUtils.getLinkedColumnListId(rootTable,
					introspectedTable.getBlobColumnListId()));
			rootTable.setFieldColumnMapField(AssociationUtils.generateFieldColumnMapField(rootTable));
			parseAssociation(fks, rootTable, rootTable, tables, false);
			introspectedTable.getTableConfiguration().setAssociationRootTable(rootTable);
		}
	}

	public void parseAssociation(List<ForeginKey> fks, AssociationRootTable parent, AssociationRootTable rootTable,
			Map<String, IntrospectedTable> tables, boolean inner_declared) {
		for (ForeginKey foreginKey : fks) {
			String ref = foreginKey.getTable();
			IntrospectedTable refTable = tables.get(ref);
			FullyQualifiedJavaType mappedType = new FullyQualifiedJavaType(refTable.getBaseRecordType());
			if (inner_declared) {
				validataForeginKey(parent, foreginKey, rootTable);
			}
			String refTableName = refTable.getFullyQualifiedTable().getIntrospectedTableName();
			AssociationTable associationTable = new AssociationTable();
			associationTable.setIntrospectedTable(refTable);
			associationTable.setName(refTableName);
			associationTable.setParent(parent);
			associationTable.setColumn(foreginKey.getKeyColumn());
			associationTable.setMappedColumn(foreginKey.getColumn());
			associationTable.setMappedType(mappedType);
			String mappedField = foreginKey.getMappedField();
			if (mappedField == null) {
				String domainObjectName = refTable.getFullyQualifiedTable().getDomainObjectName();
				mappedField = lowerCaseFirstChar(domainObjectName);
			}
			associationTable.setMappedField(parent.getUniqueField(mappedField));
			String alias = refTable.getTableConfiguration().getAlias();
			if (alias == null) {
				String domainObjectName = refTable.getFullyQualifiedTable().getDomainObjectName();
				alias = lowerCaseFirstChar(domainObjectName);
			}
			associationTable.setAlias(rootTable.getUniqueAlias(alias));
			parent.getChildren().add(associationTable);

			associationTable.setAssociationField(AssociationUtils.generateAssociationField(associationTable));
			associationTable.setAssociationFieldGetterMethod(AssociationUtils
					.generateAssociationFieldGetterMethod(associationTable));
			associationTable.setAssociationFieldSetterMethod(AssociationUtils
					.generateAssociationFieldSetterMethod(associationTable));
			associationTable.setBaseColumnListId(AssociationUtils.getLinkedColumnListId(associationTable,
					refTable.getBaseColumnListId()));
			associationTable.setBlobColumnListId(AssociationUtils.getLinkedColumnListId(associationTable,
					refTable.getBlobColumnListId()));
			associationTable.setLinkedMappedField(AssociationUtils.getLinkedMappedField(associationTable));
			associationTable.setLinkedMappedFieldInMethodFragment(AssociationUtils
					.getLinkedMappedFieldInMethodFragment(associationTable));
			associationTable.setFieldColumnMapField(AssociationUtils.generateFieldColumnMapField(associationTable));
			if (foreginKey.isUseAssociation()) {
				if (!associationTable.isParentAssociationTable(refTableName)) {
					parseAssociation(refTable.getTableConfiguration().getForeginKeys(), associationTable, rootTable,
							tables, false);
				}
			} else {
				parseAssociation(foreginKey.getChildren(), associationTable, rootTable, tables, true);
			}
		}
	}

	private void validataForeginKey(AssociationRootTable table, ForeginKey foreginKey, AssociationRootTable rootTable) {
		List<ForeginKey> fks = table.getIntrospectedTable().getTableConfiguration().getForeginKeys();
		for (ForeginKey fk : fks) {
			if (fk.getColumn().equals(foreginKey.getColumn())) {
				if (!fk.equals(foreginKey)) {
					throw new RuntimeException(table.getName()
							+ "'s foreginKey in the context don't have same attributes when parse "
							+ rootTable.getName() + "'s foreginKey");
				}
			}
		}
	}
	 ...
	 */
	public void initialize(ProgressCallback callback, List<String> warnings) throws InterruptedException {
	...
	public void generateFiles(ProgressCallback callback, List<GeneratedJavaFile> generatedJavaFiles,
		...
		if (introspectedTables != null) {
			for (IntrospectedTable introspectedTable : introspectedTables) {
				...
				generatedXmlFiles.addAll(introspectedTable.getGeneratedBlankXmlFiles());
