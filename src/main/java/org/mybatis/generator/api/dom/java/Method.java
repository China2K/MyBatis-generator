...
public class Method extends JavaElement {
	...
	private ReturnType annoReturnType;
	...
	public String getFormattedContent(int indentLevel, boolean interfaceMethod, CompilationUnit compilationUnit) {
		...
		if (!constructor) {
			if (getReturnType() == null && annoReturnType == null) {
				sb.append("void"); //$NON-NLS-1$
			} else {
				if (annoReturnType == null) {
					sb.append(JavaDomUtils.calculateTypeName(compilationUnit, getReturnType()));
				} else {
					sb.append(annoReturnType.getFormattedContent(compilationUnit));
				}
			}
			sb.append(' ');
		}
	...
	public void setReturnTypeWithAnnotation(ReturnType returnType) {
		this.annoReturnType = returnType;
	}
