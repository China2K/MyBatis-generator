...
public class MyBatisGenerator {
	...
	public MyBatisGenerator(Configuration configuration, List<String> warnings) throws InvalidConfigurationException {
		super();
		init(configuration, new DefaultShellCallback(false), warnings);
	}

	public MyBatisGenerator(Configuration configuration, boolean overwrite, List<String> warnings)
			throws InvalidConfigurationException {
		super();
		init(configuration, new DefaultShellCallback(overwrite), warnings);
	}

	public MyBatisGenerator(Configuration configuration, ShellCallback shellCallback, List<String> warnings)
			throws InvalidConfigurationException {
		super();
		init(configuration, shellCallback, warnings);
	}

	private void init(Configuration configuration, ShellCallback shellCallback, List<String> warnings)
			throws InvalidConfigurationException {
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames,
		...
		for (Context context : contextsToRun) {
			context.initialize(callback, warnings);
		}
				...
				if (shellCallback.isMergeSupported()) {
					...
					if (gjf.isOverwrite() == null) {
						...
						if (gjf.isOverwrite()) {
							...
							System.out.println("Ignore " + targetFile.getName());
						}
					}
				}
			} else {
